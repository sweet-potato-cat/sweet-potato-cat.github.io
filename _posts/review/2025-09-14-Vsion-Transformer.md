...
-> 위치 정보를 주지 않아도 이미 모델이 위치 정보를 알아서 파악했다.<br>
2. Large Datasets이 필요한 것을 알겠지만, 성능과 데이터의 반비례 관계를 지표화 시킬 수 있을까?
-> 300M 정도의 데이터셋 크기에서 비약적으로 성능이 상승한 것을 볼 수 있었다.<br>
3. 왜 16x16 패치를 사용했을까?
-> 크기를 왜 16으로 정했는지는 모르겠다, 크기가 작아지면 패치 길이가 길어져서 더 많은 정보를 학습할 수 있을 것 같다.<br>
4. Query, Key, Value를 Q: 현재 처리중인 데이터 토큰(입력), K: 다른 주변 토큰(출력), V: Q,K 사이의 유사도를 표현하는 V(가중치?) 라고 본다면 이미지 처리에서 Query, Key, Value는 어떤 의미를 가질까?
-> 우선 V는 각 이미지의 특정 값 색깔이나 모양을 나타내는 정보일 것이라고 추측할 수 있을 것 같은데, 쿼리와 키의 의미를 부여하는 것 좀 어렵다. Q는 계산중인 정보니까 랜덤 위치 또는 어떤 특징 위치라고 한다면 K는 자신들 또는 그 그룹이 가지고 있는 특징을 주지 않을까 하고 생각해봤다.<br>
